...
if __name__ == '__main__':
    ...
    model = model_whale(num_classes=num_classes,
                        inchannels=4, model_name=model_name).to(device)

    model.load_state_dict(torch.load(
        './trained_model/{}.model'.format(model_name)))
    ...
    dataset_test = WhaleDatasetTest(
        images_test, transforms_dict['val'])
    ...
    testdf.to_csv(
        './results/submission_{}.csv'.format(model_name), index=False)
